...
REM 只有go1.10才能在windows xp运行
set "GO_BIN=W:\sdk\GO\go1.10\bin\go.exe"
set "GO_DIR=p2p_client"
set "GO_EXE=p2p_client.exe"
REM ============================================
...
REM 执行 go mod tidy（Go 1.10 不支持 modules，跳过）
if %DO_TIDY%==1 (
    echo [WARN] Go 1.10 does not support 'go mod tidy'. Skipping.
)
